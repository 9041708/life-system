...
| 🎲 **我的人生** | 人生模拟游戏，0~99岁随机事件，性别选择，属性成长，事件不可重复触发，成就系统，80+事件覆盖全年龄段 |
| 🎮 **娱乐** | 虚拟股票交易，40只个股，K线图，限价委托，破产借贷，排行榜 |
| 📋 **考勤** | 排班管理+出勤日历+请假管理（小时级）+薪资计算+绩效管理+扣款+社保公积金，企业入职/离职追踪 |
...
部署后享有 **30天完整试用期**。试用期结束后：
- ✅ 记账、规划、负债、报销 仍可正常使用
- 🔒 其余菜单组需获取授权码激活

获取授权码：访问主站 `/license/` 页面，填写邮箱、域名、选择方案并上传付款截图，管理员审批后发放授权码到您的邮箱。

在您的站点：点击顶部横幅「激活授权」→ 输入授权服务器地址和授权码 → 激活。
