// miniprogram/pages/budget/list.ts
...
Page({
  data: {
    ...
    // 主 Tab
    mainTab: 'subscription' as 'subscription' | 'asset',

    // ---- 订阅相关（来自 subscriptions/list.ts）----
    loading: false,
    keyword: '',
    ...
    // ---- 资产相关（来自 assets/list.ts）----
    assetTab: 'active' as 'active' | 'transferred',
    assetsActive: [] as any[],
  ...
  // ========== 主 Tab 切换 ==========
  onMainTabSubscription() {
    if (this.data.mainTab === 'subscription') return;
    this.setData({ mainTab: 'subscription' });
  },

  onMainTabAsset() {
    if (this.data.mainTab === 'asset') return;
    this.setData({ mainTab: 'asset' });
  },

  // ========== 订阅逻辑 ==========
  async reload() {
    if (this.data.mainTab === 'subscription') {
  ...
  onTabActive() {
    if (this.data.assetTab === 'active') return;
    this.setData({ assetTab: 'active' });
  },

  onTabTransferred() {
    if (this.data.assetTab === 'transferred') return;
    this.setData({ assetTab: 'transferred' });
  },
