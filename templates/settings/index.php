...
<?php elseif ($tab === 'system' && $isAdmin): ?>
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        <?php foreach ($bgImages as $bg): ?>
                            <div class="position-relative border rounded p-1 <?= ($system['bg_image_path'] ?? '') === $bg['file_path'] ? 'border-primary' : '' ?>" style="background:rgba(255,255,255,0.5);">
                                <img src="/uploads/<?= htmlspecialchars($bg['file_path']) ?>" alt="背景图" style="width:100px;height:60px;object-fit:cover;border-radius:4px;cursor:pointer;" onclick="selectBg('<?= htmlspecialchars($bg['file_path']) ?>')">
    ...
    <script>
    function selectBg(path){
        var fd=new FormData();fd.append('action','select_bg');fd.append('bg_path',path);
        fetch('/public/index.php?route=settings&action=select_bg',{method:'POST',headers:{'X-Requested-With':'XMLHttpRequest'},body:fd})
