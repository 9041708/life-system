...
<div class="ent-ban">🎮 仅供娱乐，实际投资请注意股市风险 <span class="ms-2" id="tradeStatus"><?= $isTradeTime?'🟢交易中':'🔴休市' ?></span>
    <div style="display:inline-flex;align-items:center;gap:2px;margin-left:12px;height:16px;width:200px;background:#e5e7eb;border-radius:3px;overflow:hidden;position:relative;vertical-align:middle">
        <div style="width:16.7%;height:100%;background:#22c55e" title="上午盘 08:00-12:00"></div>
        <div style="width:4.2%;height:100%;background:#f59e0b" title="午休 12:00-13:00"></div>
        <div style="width:16.7%;height:100%;background:#22c55e" title="下午盘 13:00-17:00"></div>
        <div style="width:8.3%;height:100%;background:#f59e0b" title="晚休 17:00-19:00"></div>
        <div style="width:12.5%;height:100%;background:#22c55e" title="晚间盘 19:00-22:00"></div>
        <div style="width:41.6%;height:100%;background:#374151" title="深夜休市 22:00-08:00"></div>
...
<div class="ent-wrap">
    ...
    <div class="ent-right">
        <div class="ent-detail" id="detail"><div class="ent-dh"><div><div class="fw-bold" id="dn">-</div><div class="text-muted small" id="ds">-</div></div><div class="text-end"><div class="ep ent-flat" id="dp">-</div><div class="ec ent-flat" id="dc">-</div></div></div><div class="ent-info-grid" id="infoGrid"></div>
...
<div class="ent-panel" id="pn-acc">
    <div class="row g-3">
        <div class="col-md-4"><div class="card border-0 shadow-sm"><div class="card-body text-center"><div class="fs-4 fw-bold"><?=number_format($totalAssets,2)?></div><div class="text-muted small">总资产</div></div></div></div>
        <div class="col-md-4"><div class="card border-0 shadow-sm"><div class="card-body text-center"><div class="fs-4 fw-bold"><?=number_format($acc['balance'],2)?></div><div class="text-muted small">可用</div></div></div></div>
        <div class="col-md-4"><div class="card border-0 shadow-sm"><div class="card-body text-center"><div class="fs-4 fw-bold"><?=number_format($totalMV,2)?></div><div class="text-muted small">市值</div></div></div></div>
        <div class="col-md-4"><div class="card border-0 shadow-sm"><div class="card-body text-center"><div class="fs-4 fw-bold <?=$totalProfit>=0?'ent-up':'ent-down'?>"><?=$totalProfit>=0?'+':''?><?=number_format($totalProfit,2)?></div><div class="text-muted small">盈亏</div></div></div></div>
...
<div class="ent-panel" id="pn-rank"><table class="table table-sm"><thead><tr><th>#</th><th>昵称</th><th class="text-end">资产</th></tr></thead><tbody>
<?php $rk=0;foreach($lb as $r):$rk++;?><tr><td><?=$rk?></td><td><?=htmlspecialchars($r['nickname']?:$r['username']?:'用户'.$r['user_id'])?></td><td class="text-end fw-bold"><?=number_format($r['balance'],2)?></td></tr><?php endforeach;?></tbody></table></div>
...
<?php foreach($adminStocks as $s):?><tr><td><?=$s['symbol']?></td><td><?=htmlspecialchars($s['name'])?></td><td class="text-end"><?=number_format($s['current_price'],2)?></td><td class="text-muted small"><?=htmlspecialchars(mb_substr((string)($s['description']??''),0,15))?></td><td><button class="btn btn-sm btn-outline-primary py-0" style="font-size:0.65rem" onclick='editStock(<?=json_encode($s,JSON_UNESCAPED_UNICODE|JSON_HEX_APOS)?>)'>编辑</button></td></tr><?php endforeach;?></tbody></table></details>
...
<details><summary class="fw-semibold small mt-2" style="cursor:pointer">📰 新闻管理 (<?=count($adminNews)?>条)</summary>
    ...
    <?php endif;?></details>
<details><summary class="fw-semibold small mt-2" style="cursor:pointer">📢 系统公告</summary><textarea id="noticeTxt" class="form-control form-control-sm mb-1" rows="2"><?=htmlspecialchars($sysNotice)?></textarea><button class="btn btn-sm btn-primary" onclick="saveNotice()">保存</button></details>
...
<details><summary class="fw-semibold small mt-2" style="cursor:pointer">⚙️ 借贷管理 (<?= count($adminLoans) ?>条)</summary>
    <button class="btn btn-sm btn-primary mb-2" onclick="openLoanConfig()">借贷配置</button>
    <?php if(empty($adminLoans)):?><div class="text-muted small">暂无借贷记录</div><?php else:?>
    <table class="table table-sm"><thead><tr><th>用户</th><th>金额</th><th>利率</th><th>方式</th><th>应还</th><th>已还</th><th>期限</th><th>状态</th></tr></thead><tbody>
    <?php foreach($adminLoans as $ln):$ms=['equal'=>'等额本息','interest_first'=>'先息后本','equal_principal'=>'等额本金'];$ss=['active'=>'正常','repaid'=>'已还','overdue'=>'逾期'];?>
    <tr><td><?=htmlspecialchars($ln['nickname']?:$ln['username']?:'UID'.$ln['user_id'])?></td><td><?=number_format($ln['amount'],0)?></td><td><?=$ln['interest_rate']?>%</td><td><?=$ms[$ln['repay_method']]?></td><td><?=number_format($ln['total_repayable'],2)?></td><td><?=number_format($ln['repaid'],2)?></td><td><?=$ln['due_date']?></td><td><span class="badge bg-<?=$ln['status']==='repaid'?'success':($ln['status']==='overdue'?'danger':'warning')?>"><?=$ss[$ln['status']]?></span></td></tr>
    <?php endforeach;?></tbody></table><?php endif;?></details>
</div>
...
<!-- 借贷配置弹窗 -->
<div class="modal fade" id="loanConfigModal" tabindex="-1"><div class="modal-dialog modal-dialog-centered modal-sm"><div class="modal-content">
<div class="modal-header py-2"><h6 class="modal-title">借贷配置</h6><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
<div class="modal-body">
    <label class="form-label small mb-1">借贷开关</label>
    <select id="cfLoan" class="form-select form-select-sm mb-2"><option value="1" <?=$loanEnabled?'selected':''?>>开启</option><option value="0" <?=!$loanEnabled?'selected':''?>>关闭</option></select>
    <label class="form-label small mb-1">单笔借贷上限</label>
    <input id="cfMax" class="form-control form-control-sm mb-2" value="<?=$loanMax?>">
    <label class="form-label small mb-1">年利率 (%)</label>
    <input id="cfRate" class="form-control form-control-sm mb-3" value="<?=$loanRate?>">
    <button class="btn btn-sm btn-primary w-100" onclick="saveCfg()">保存配置</button>
</div></div></div></div>
<?php endif;?>

<script>
var curSid=0,curPrice=0,chart=null,candle=null,balance=<?=(float)$acc['balance']?>,kRange=30,fullScreen=false,posData=<?=json_encode($positions,JSON_UNESCAPED_UNICODE|JSON_HEX_APOS|JSON_HEX_QUOT)?>;
...
function loadNews(id){
    var newsEl=document.getElementById('stockNews'),html='';
    <?php foreach($allNews as $n):?>
    if(parseInt('<?=$n['stock_id']?>')===id){var type='<?=$n['effect']?>',cls=type==='positive'?'good':'bad';
        html+='<div class="ent-news-bar '+cls+'"><span>'+(type==='positive'?'📈':'📉')+'</span><span><?=htmlspecialchars(addslashes($n['title']))?></span><span class="text-muted small">强度:<?=$n['strength']?></span></div>';}
    <?php endforeach;?>
...
function openNewsModal(){document.getElementById('nsTitle').value='';document.getElementById('nsSearch').value='';document.getElementById('nsSched').value='';new bootstrap.Modal(document.getElementById('newsModal')).show();setTimeout(function(){var ns=document.getElementById('nsSearch');if(ns&&!ns._bound){ns.addEventListener('input',filterNsStocks);ns._bound=true;}filterNsStocks();},200);}
function openLoanConfig(){new bootstrap.Modal(document.getElementById('loanConfigModal')).show();}
...
function filterNsStocks(){
    ...
    var items=document.querySelectorAll('#newsModal .ns-item');
    for(var i=0;i<items.length;i++){
        var el=items[i];
        if(!kw){el.style.display='';continue;}
        var t=(el.textContent||'').toLowerCase();
        el.style.display=t.indexOf(kw)!==-1?'':'none';
    }
    }catch(e){}
}
...
function saveCfg(){var f=new FormData();f.append('action','save_config');f.append('loan_enabled',document.getElementById('cfLoan').value);f.append('loan_max',document.getElementById('cfMax').value);f.append('loan_rate',document.getElementById('cfRate').value);fetch('/public/index.php?route=entertainment-api',{method:'POST',body:f}).then(function(r){return r.json()}).then(function(d){alert(d.message||d.error);if(d.ok){var m=document.getElementById('loanConfigModal');if(m){m.classList.remove('show');m.style.display='none';document.body.classList.remove('modal-open');var bd=document.querySelector('.modal-backdrop');if(bd)bd.remove();}location.reload();}});}
...
function isTradeTime(){var h=(new Date()).getHours(),m=(new Date()).getMinutes(),t=h*60+m;return(t>=480&&t<720)||(t>=780&&t<1020)||(t>=1140&&t<1320);}
function updateTimeMarker(){var n=new Date(),h=n.getHours(),m=n.getMinutes(),t=h*60+m,left=0;if(t>=480&&t<720)left=((t-480)/240)*16.7;else if(t>=720&&t<780)left=16.7+((t-720)/60)*4.2;else if(t>=780&&t<1020)left=20.9+((t-780)/240)*16.7;else if(t>=1020&&t<1140)left=37.6+((t-1020)/120)*8.3;else if(t>=1140&&t<1320)left=45.9+((t-1140)/180)*12.5;else if(t>=1320)left=58.4+((t-1320)/120)*41.6;else left=(t/480)*16.7;document.getElementById('timeMarker').style.left=Math.min(98,left)+'%';document.getElementById('tradeStatus').textContent=isTradeTime()?'🟢交易中':'🔴休市';}
