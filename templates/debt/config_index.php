...
<?php else: ?>
    ...
    ?>
    <?php
    $activeList = array_filter($filteredConfigs, function($c) { return ($c['progress_percent'] ?? 0) < 100; });
    $completedList = array_filter($filteredConfigs, function($c) { return ($c['progress_percent'] ?? 0) >= 100; });
    ?>
    <?php if (!empty($activeList)): ?>
    <div class="mb-1 small fw-semibold">📋 进行中</div>
    <div class="row g-2 mb-3">
        <?php foreach ($activeList as $config): $statusBadge='success'; $statusText='进行中'; $methodText=$config['repayment_method']==='equal'?'等额本息':'等额本金'; ?>
    ...
    <details class="mb-3">
        <summary class="small fw-semibold text-muted" style="cursor:pointer">✅ 已结清 (<?= count($completedList) ?>)</summary>
        <div class="row g-2 mt-2">
        <?php foreach ($completedList as $config): $statusBadge='secondary'; $statusText='已结清'; $methodText=$config['repayment_method']==='equal'?'等额本息':'等额本金'; ?>
            <div class="col-md-6 col-lg-4">
                <div class="card border-0 shadow-sm h-100" style="font-size:0.85rem;opacity:0.75">
                    <div class="card-body py-2 px-3">
                        <div class="d-flex justify-content-between align-items-start mb-1">
                            <div class="d-flex align-items-center gap-2">
                                <span class="badge bg-<?= $statusBadge ?>" style="font-size:0.65rem;"><?= $statusText ?></span>
                        ...
                        <div class="d-flex align-items-center gap-1">
                            <div class="progress flex-grow-1" style="height:4px;"><div class="progress-bar bg-success" style="width:100%"></div></div>
                            <span style="font-size:0.7rem;">100%</span>
                        </div>
                    </div>
                </div>
            </div>
