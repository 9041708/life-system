<form method="post" id="debtConfigForm">
<?php if ($isEdit && $config): ?>
                        ...
                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-primary btn-lg" id="debtSubmitBtn">
                                <?= $isEdit ? '更新配置' : '创建配置' ?>
                            </button>
                            <a href="/public/index.php?route=debt-config" class="btn btn-outline-secondary">← 返回列表</a>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
// 防重复提交
document.getElementById('debtConfigForm').addEventListener('submit', function() {
    var btn = document.getElementById('debtSubmitBtn');
