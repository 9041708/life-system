<style>
/* 筛选卡片 */
.tx-filter-card .form-select,
.tx-filter-card .form-control {
    border-radius: 0.6rem !important;
    font-size: 0.84rem;
}
.tx-filter-card .btn-primary {
    border-radius: 0.6rem !important;
}
...
.tx-list-table .tx-icon-cell {
    display: inline-flex; align-items: center; justify-content: center;
    width: 24px; height: 24px; flex-shrink: 0;
    vertical-align: middle;
}
.tx-list-table .tx-icon-cell img {
    width: 20px; height: 20px; object-fit: cover; border-radius: 4px;
}
.tx-list-table .tx-icon-cell svg {
    width: 18px; height: 18px;
}
.tx-list-table .tx-attach-cell {
    display: inline-flex; align-items: center; gap: 4px;
    min-height: 36px;
}
.tx-list-table .tx-attach-cell img {
    width: 36px; height: 36px; object-fit: cover; border-radius: 6px; cursor: zoom-in;
}
...
#txDetailModal .modal-content,
#modalTxEditFromList .modal-content {
    position: relative;
    z-index: 1;
    pointer-events: auto;
...
body.theme-dark #txDetailModal .modal-content,
body.theme-dark #modalTxEditFromList .modal-content {
    position: relative;
    z-index: 1;
    pointer-events: auto;
...
#txDetailModal .modal-header,
#modalTxEditFromList .modal-header { border-bottom-color: rgba(15,23,42,0.05); background: transparent; }
body.theme-dark #txDetailModal .modal-header,
body.theme-dark #modalTxEditFromList .modal-header { border-bottom-color: rgba(148,163,184,0.1); }
#txDetailModal .modal-footer,
#modalTxEditFromList .modal-footer { border-top-color: rgba(15,23,42,0.05); background: transparent; }
body.theme-dark #txDetailModal .modal-footer,
body.theme-dark #modalTxEditFromList .modal-footer { border-top-color: rgba(148,163,184,0.1); }
#txDetailModal .modal-body,
#modalTxEditFromList .modal-body { background: transparent; }
#txDetailModal .modal-dialog,
#modalTxEditFromList .modal-dialog {
    position: relative;
    z-index: 2;
    pointer-events: auto;
}
...
<form method="get" action="/public/index.php" class="card glass-card mb-4 tx-filter-card">
    <div class="card-body p-3">
        ...
        <div class="col-6 col-md-2">
            <label class="form-label small">每页</label>
            <?php
            $pageSize = (int)($pageSize ?? 10);
            $pageSizeOptions = [10, 30, 50, 100];
            if (!in_array($pageSize, $pageSizeOptions, true)) { $pageSize = 10; }
            ?>
            <select name="page_size" class="form-select form-select-sm" id="txPageSizeSelect">
                <?php foreach ($pageSizeOptions as $opt): ?>
                    <option value="<?= (int)$opt ?>" <?= $pageSize === (int)$opt ? 'selected' : '' ?>><?= (int)$opt ?> 条</option>
                <?php endforeach; ?>
            </select>
        </div>
...
<form id="txBulkForm" method="post" action="/public/index.php?route=transaction-delete" class="card glass-card mb-3">
    <div class="card-body p-3">
        <?php if (!empty($summary)): ?>
            <div class="alert alert-light border mb-3 small py-2">
                <span class="me-3 fw-semibold">筛选汇总：</span>
                <span class="me-3">收入 <span class="text-danger fw-semibold">¥ <?= number_format((float)($summary['income'] ?? 0), 2) ?></span></span>
                <span class="me-3">支出 <span class="text-success fw-semibold">¥ <?= number_format((float)($summary['expense'] ?? 0), 2) ?></span></span>
                <?php if (!empty($transferEnabled)): ?>
                    <span>转账合计：<span class="text-secondary">¥ <?= number_format((float)($summary['transfer'] ?? 0), 2) ?></span></span>
                <?php endif; ?>
                <button type="submit" id="txBulkDeleteBtnSummary" class="btn btn-sm btn-outline-danger ms-2 d-none">批量删除选中</button>
            </div>
        <?php endif; ?>
        <div class="table-responsive">
            <table class="table table-sm align-middle mb-0 tx-list-table">
                <thead class="table-light">
                <tr id="txBulkHeaderActionRow" class="d-none">
                    <th colspan="11" class="text-center">
                        <button type="submit" id="txBulkDeleteBtnHeader" class="btn btn-sm btn-outline-danger">批量删除选中</button>
                    </th>
                </tr>
                <tr>
                ...
                <?php if (empty($transactions)): ?>
                    <tr><td colspan="11" class="text-center text-muted small">暂无记录</td></tr>
                <?php else: ?>
                    <?php foreach ($transactions as $t): ?>
                        <tr>
                            <td><input type="checkbox" class="form-check-input tx-check" name="ids[]" value="<?= (int)$t['id'] ?>"></td>
                            <td class="small text-muted"><?= (int)$t['id'] ?></td>
                            <td>
                                <?php if ($t['type'] === 'income'): ?>
                                    <span class="badge bg-danger bg-opacity-15 text-danger">收入</span>
                                <?php elseif ($t['type'] === 'expense'): ?>
                                    <span class="badge bg-success bg-opacity-15 text-success">支出</span>
                                <?php else: ?>
                                    <span class="badge bg-secondary bg-opacity-15 text-secondary">转账</span>
                                <?php endif; ?>
                            </td>
                            <td>
                                <span class="tx-icon-cell">
                                <?php if (!empty($t['category_icon_type']) && !empty($t['category_icon_value'])): ?>
                                    <?php if ($t['category_icon_type'] === 'file'): ?>
                                        <img src="/uploads/<?= htmlspecialchars($t['category_icon_value'], ENT_QUOTES) ?>" alt="">
                                    <?php elseif ($t['category_icon_type'] === 'svg'): ?>
                                        <?= $t['category_icon_value'] ?>
                                    <?php endif; ?>
                                <?php endif; ?>
                                </span>
                                <span class="ms-1"><?= htmlspecialchars($t['category_name'] ?? '') ?></span>
                            </td>
                            <td>
                                <span class="tx-icon-cell">
                                <?php if (!empty($t['item_icon_type']) && !empty($t['item_icon_value'])): ?>
                                    <?php if ($t['item_icon_type'] === 'file'): ?>
                                        <img src="/uploads/<?= htmlspecialchars($t['item_icon_value'], ENT_QUOTES) ?>" alt="">
                                    <?php elseif ($t['item_icon_type'] === 'svg'): ?>
                                        <?= $t['item_icon_value'] ?>
                                    <?php endif; ?>
                                <?php endif; ?>
                                </span>
                                <span class="ms-1"><?= htmlspecialchars($t['item_name'] ?? '') ?></span>
                            </td>
                            <td>
                                <span class="tx-icon-cell">
                                <?php if ($t['type'] === 'transfer'): ?>
                                    <?php if (!empty($t['from_account_icon_type']) && !empty($t['from_account_icon_value'])): ?>
                                        <?php if ($t['from_account_icon_type'] === 'file'): ?>
                                            <img src="/uploads/<?= htmlspecialchars($t['from_account_icon_value'], ENT_QUOTES) ?>" alt="">
                                ...
                                <?php else: ?>
                                    <?php if (!empty($t['to_account_icon_type']) && !empty($t['to_account_icon_value'])): ?>
                                        <?php if ($t['to_account_icon_type'] === 'file'): ?>
                                            <img src="/uploads/<?= htmlspecialchars($t['to_account_icon_value'], ENT_QUOTES) ?>" alt="">
                                ...
                                <span class="ms-1">
                                <?php if ($t['type'] === 'transfer'): ?>
                                    <?= htmlspecialchars($t['from_account_name'] ?? '') ?> → <?= htmlspecialchars($t['to_account_name'] ?? '') ?>
                                <?php elseif ($t['type'] === 'expense'): ?>
                                    <?= htmlspecialchars($t['from_account_name'] ?? '') ?>
                                <?php else: ?>
                                    <?= htmlspecialchars($t['to_account_name'] ?? '') ?>
                                <?php endif; ?>
                                </span>
                            </td>
                            <td class="text-end">
                                ...
                                <?php else: ?>
                                    <span><?= number_format($t['amount'], 2) ?></span>
                                <?php endif; ?>
                            </td>
                            ...
                            <td>
                                <?php
                                $thumbAttachments = $t['attachments'] ?? [];
                                if (empty($thumbAttachments) && !empty($t['attachment_path'])) {
                                    $thumbAttachments = [(string)$t['attachment_path']];
                                }
                                ?>
                                <div class="tx-attach-cell">
                                <?php if (!empty($thumbAttachments)): ?>
                                    <?php $firstPath = (string)$thumbAttachments[0]; ?>
                                    <img src="/uploads/<?= htmlspecialchars($firstPath, ENT_QUOTES) ?>"
                                         alt="凭证"
                                         data-attachment-preview="/uploads/<?= htmlspecialchars($firstPath, ENT_QUOTES) ?>">
                                    <?php if (count($thumbAttachments) > 1): ?>
                                        <span class="badge bg-secondary bg-opacity-75 small">+<?= count($thumbAttachments) - 1 ?></span>
                                    <?php endif; ?>
                                <?php else: ?>
                            ...
                            <td class="text-center">
                                <button type="button"
                                        ...
                                        data-account-name="<?php
                                            if ($t['type'] === 'transfer') {
                                                echo htmlspecialchars((($t['from_account_name'] ?? '') . ' → ' . ($t['to_account_name'] ?? '')), ENT_QUOTES);
                                            } elseif ($t['type'] === 'expense') {
                                                echo htmlspecialchars(($t['from_account_name'] ?? ''), ENT_QUOTES);
                                            } else {
                                                echo htmlspecialchars(($t['to_account_name'] ?? ''), ENT_QUOTES);
                                            }
                                        ?>"
                                        data-account-balance="<?php
                                            if ($t['type'] === 'transfer') {
                                                echo '';
                                            } elseif ($t['type'] === 'expense') {
                                                echo htmlspecialchars((string)($t['from_account_balance'] ?? ''), ENT_QUOTES);
                                            } else {
                                                echo htmlspecialchars((string)($t['to_account_balance'] ?? ''), ENT_QUOTES);
                                            }
                                        ?>"
                                        ...
                                        data-attachments='<?= htmlspecialchars(json_encode(array_values(array_map(static fn($p) => '/uploads/' . ltrim((string)$p, '/\\'), $thumbAttachments ?? [])), JSON_UNESCAPED_SLASHES), ENT_QUOTES) ?>'>
                                    详情
                                </button>
                                <button type="button"
    ...
    $queryBaseParams = array_merge(['route' => 'transactions'], $filters ?? [], ['page_size' => $pageSize]);
    $base = '/public/index.php?' . http_build_query($queryBaseParams) . '&page=';
    $prevPage = $pageIndex > 1 ? $pageIndex - 1 : 1;
    $nextPage = $pageIndex < $totalPages ? $pageIndex + 1 : $totalPages;
    ?>
    <div class="px-3 pb-2">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mt-2 gap-2">
            <div class="small text-muted">第 <?= (int)$pageIndex ?> / <?= (int)$totalPages ?> 页，共 <?= (int)$totalCount ?> 条</div>
            <nav aria-label="明细分页">
                <ul class="pagination pagination-sm mb-0">
                    ...
                    <?php
                    $startPage = max(1, $pageIndex - 2);
                    $endPage = min($totalPages, $pageIndex + 2);
                    if ($startPage > 1): ?>
                    ...
                    <?php for ($i = $startPage; $i <= $endPage; $i++): ?>
                        <li class="page-item <?= $i == $pageIndex ? 'active' : '' ?>">
                            <a class="page-link" href="<?= htmlspecialchars($base . $i) ?>"><?= $i ?></a>
                        </li>
                    <?php endfor; ?>
                    ...
                    <div class="row g-3">
                        <div class="col-12 col-md-3">
                            ...
                            <select name="type" class="form-select form-select-sm" id="txEditTypeList">
                                ...
                                <option value="transfer" <?= empty($transferEnabled) ? 'disabled' : '' ?>>转账</option>
                            </select>
                        </div>
                        <div class="col-12 col-md-3">
                            <label class="form-label small">分类</label>
...
document.addEventListener('DOMContentLoaded', function () {
    var txTransferEnabled = <?= !empty($transferEnabled) ? 'true' : 'false' ?>;
    // 每页条数：切换后回到第 1 页
    (function () {
        var sel = document.getElementById('txPageSizeSelect');
        if (!sel) return;
        sel.addEventListener('change', function () {
            // 直接提交筛选表单（表单已内置 page=1）
            var form = sel.closest('form');
            if (form) form.submit();
        });
    })();

    // 批量删除按钮：勾选后在汇总区与表头中间显示
    (function () {
        var form = document.getElementById('txBulkForm');
        if (!form) return;

        var summaryBtn = document.getElementById('txBulkDeleteBtnSummary');
        var headerRow = document.getElementById('txBulkHeaderActionRow');
        var headerBtn = document.getElementById('txBulkDeleteBtnHeader');

        function updateBulkDeleteUI() {
            var checked = form.querySelectorAll('input.tx-check:checked').length;
            var show = checked > 0;

            if (summaryBtn) {
                summaryBtn.classList.toggle('d-none', !show);
                summaryBtn.textContent = show ? ('批量删除选中（' + checked + '）') : '批量删除选中';
            }
            if (headerRow) {
                headerRow.classList.toggle('d-none', !show);
            }
            if (headerBtn) {
                headerBtn.textContent = show ? ('批量删除选中（' + checked + '）') : '批量删除选中';
            }
        }

        form.addEventListener('change', function (e) {
            var t = e.target;
            if (t && t.classList && t.classList.contains('tx-check')) {
                updateBulkDeleteUI();
            }
        });
    ...
    (function () {
        ...
        modalEl.addEventListener('show.bs.modal', function (event) {
            ...
            var attachmentsJson = button.getAttribute('data-attachments') || '';
            var attachmentUrls = [];
            if (attachmentsJson) {
                try {
                    var parsed = JSON.parse(attachmentsJson);
                    if (Array.isArray(parsed)) {
                        attachmentUrls = parsed.filter(function (u) { return typeof u === 'string' && u !== ''; });
                    }
                } catch (e) {
                    attachmentUrls = [];
                }
            ...
            document.getElementById('txDetailAmount').textContent = amountPrefix + (amount || '0.00');
            ...
            if (attachWrapper) {
                ...
                if (attachmentUrls.length > 0) {
                    attachmentUrls.forEach(function (url) {
                        ...
                        img.src = url;
                        img.alt = '凭证';
                        img.className = 'attachment-thumb me-2 mb-2';
                        img.style.maxWidth = '120px';
                        img.style.maxHeight = '120px';
                        img.style.objectFit = 'cover';
                        img.style.borderRadius = '4px';
                        img.style.cursor = 'zoom-in';
                        img.setAttribute('data-attachment-preview', url);
    ...
    // 将弹窗移到 <body> 末尾，确保与 modal-backdrop 处在同一层叠上下文
    if (modalEl.parentNode !== document.body) {
        document.body.appendChild(modalEl);
    }
    var detailModal = document.getElementById('txDetailModal');
    ...
    modalEl.addEventListener('show.bs.modal', function (event) {
        ...
        // 兼容：若用户关闭了“转账功能”，历史转账流水仍允许编辑并保持为转账
        if (typeSelect) {
            var transferOpt = typeSelect.querySelector('option[value="transfer"]');
            if (transferOpt) {
                transferOpt.disabled = (!txTransferEnabled) && (type !== 'transfer');
            }
        }
        ...
        if (timeInput) {
            if (transTime) {
                var normalizedTime = transTime.replace(/\//g, '-');
                if (normalizedTime.indexOf('T') === -1) {
                    normalizedTime = normalizedTime.replace(' ', 'T');
                }
                if (normalizedTime.length > 16) {
                    normalizedTime = normalizedTime.substring(0, 16);
                }
...
// 编辑弹窗：类型切换时过滤分类（支出只显示支出分类，收入只显示收入分类）
(function(){
    var et=document.getElementById('txEditTypeList'),ec=document.getElementById('txEditCategoryList');
    if(!et||!ec)return;
    var allOpts=Array.from(ec.options).slice(1); // 跳过"请选择"
    et.addEventListener('change',function(){
        var v=et.value;
        ec.querySelectorAll('option').forEach(function(o){if(o.value==='')return;o.style.display=o.getAttribute('data-type')===v?'':'none';});
        if(ec.value&&ec.options[ec.selectedIndex]?.getAttribute('data-type')!==v)ec.value='';
    });
})();
</script>
