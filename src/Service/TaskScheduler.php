...
class TaskScheduler
{
    ...
    {
        $results = ['signin' => 0, 'reply' => 0, 'notice' => 0, 'cleaned' => 0, 'errors' => []];
        ...
        try {
            ...
            foreach ($replyAccounts as $account) {
                try {
                    ...
                    if (empty($message)) {
                        $message = \App\Service\DiscuzService::getReplyContent($replyMode, $customReply);
                    }
        ...
        return [
            'message' => "签到{$results['signin']}个，回帖{$results['reply']}个，通知{$results['notice']}个，清理{$results['cleaned']}条日志",
            'details' => $results,
