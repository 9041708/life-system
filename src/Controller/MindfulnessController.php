...
class MindfulnessController
{
    ...
    public function checkin(): void
    {
        ...
        // 自动签到
        $today = date('Y-m-d');
        if (!empty($config['auto_checkin']) && !MindfulnessCheckin::isCheckedIn($userId, $today)) {
            MindfulnessCheckin::checkin($userId, $today, (float)$config['checkin_score']);
        }
    ...
    {
        $data = [];
        if (isset($_POST['initial_score'])) $data['initial_score'] = (float)$_POST['initial_score'];
        if (isset($_POST['checkin_score'])) $data['checkin_score'] = (float)$_POST['checkin_score'];
        if (isset($_POST['auto_checkin'])) $data['auto_checkin'] = (int)$_POST['auto_checkin'];
        if (isset($_POST['ai_mode'])) $data['ai_mode'] = in_array($_POST['ai_mode'], ['system', 'custom']) ? $_POST['ai_mode'] : 'system';
        if (isset($_POST['custom_api_url'])) $data['custom_api_url'] = trim($_POST['custom_api_url']);
        if (isset($_POST['custom_api_key'])) $data['custom_api_key'] = trim($_POST['custom_api_key']);
        if (isset($_POST['custom_model'])) $data['custom_model'] = trim($_POST['custom_model']);

        if (isset($_POST['pi_name'])) {
            $positiveItems = []; $piNames = $_POST['pi_name']; $piScores = $_POST['pi_score'] ?? [];
            for ($i = 0; $i < count($piNames); $i++) { $name = trim($piNames[$i]); if ($name !== '') $positiveItems[$name] = (float)($piScores[$i] ?? 0); }
            $data['positive_items'] = $positiveItems;
        }
        if (isset($_POST['ni_name'])) {
            $negativeItems = []; $niNames = $_POST['ni_name']; $niScores = $_POST['ni_score'] ?? [];
            for ($i = 0; $i < count($niNames); $i++) { $name = trim($niNames[$i]); if ($name !== '') $negativeItems[$name] = (float)($niScores[$i] ?? 0); }
            $data['negative_items'] = $negativeItems;
        }
        if (isset($_POST['br_days'])) {
            $bonusRules = []; $brDays = $_POST['br_days']; $brBonus = $_POST['br_bonus'] ?? [];
            for ($i = 0; $i < count($brDays); $i++) { $days = (int)($brDays[$i] ?? 0); $bonus = (float)($brBonus[$i] ?? 0); if ($days > 0) $bonusRules[] = ['days' => $days, 'bonus' => $bonus]; }
