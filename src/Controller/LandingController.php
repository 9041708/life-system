...
class LandingController
{
    ...
    public function submitLicenseRequest(): void
    {
        ...
        if ($type === 'first') {
            ...
            if ($oldDomain !== '' || $newDomain !== '') {
                $note = '更换授权：原域名 ' . ($oldDomain !== '' ? $oldDomain : '（未填）') . '，新域名 ' . ($newDomain !== '' ? $newDomain : '（未填）');
            }
        }
        ...
        $isFreeChange = false;
        $baseLicense = null;
        if ($type === 'change' && $email !== '' && $oldDomain !== '') {
            try {
                $baseLicense = LicenseUser::findByEmailAndDomain($email, $oldDomain);
            } catch (\Throwable $e) {
                $baseLicense = null;
            }
            if ($baseLicense) {
                $quota = (int)($baseLicense['domain_change_quota'] ?? 0);
                $used = (int)($baseLicense['domain_change_used'] ?? 0);
        ...
        if ($email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $error = '请填写有效的邮箱地址';
        } elseif ($type === 'first' && $domainFirst === '') {
            $error = '请填写授权域名';
        } elseif ($type === 'change' && ($oldDomain === '' || $newDomain === '')) {
            $error = '请填写原域名和新域名';
        } elseif ($type === 'change' && empty($baseLicense)) {
            $error = '未找到该邮箱与原授权域名对应的授权记录，请确认后再提交';
        } elseif ($type === '') {
            $error = '请选择申请类型';
        } elseif ($type === 'first' && $period === null) {
            $error = '请选择授权周期';
        } elseif (!$isFreeChange && $payMethod === '') {
            $error = '请选择支付方式';
        } elseif (!$isFreeChange && ($payProofRelative === null || $payProofRelative === '')) {
            $error = '请上传付款成功截图';
        }
    ...
    public function queryLicense(): void
    {
        ...
        $changeQuota = (int)($license['domain_change_quota'] ?? 0);
        $changeUsed = (int)($license['domain_change_used'] ?? 0);
        ...
        echo json_encode([
            ...
            'data' => [
                ...
                'period' => $periodLabel,
                'expire_at' => $expireAt,
                'domain_change_quota' => $changeQuota,
                'domain_change_used' => $changeUsed,
            ],
        ]);
    }
}
