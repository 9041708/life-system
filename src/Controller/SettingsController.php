...
class SettingsController
{
    ...
    public function index(): void
    {
            ...
            if ($action === 'update_profile') {
                ...
                if ($user) {
                    ...
                    $html = '<div style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto;padding:20px;border:1px solid #e0e0e0;border-radius:8px">' .
                        '<h2 style="color:#333;margin:0 0 12px">SanS三石记账</h2>' .
                        '<p>您好，' . htmlspecialchars($user['nickname']) . '：</p>' .
                        '<p>管理员已为您重置登录密码。</p>' .
                        '<div style="background:#fff3e0;border:2px dashed #ff9800;border-radius:8px;padding:16px;text-align:center;margin:16px 0">' .
                        '<div style="font-size:0.8rem;color:#999;margin-bottom:4px">您的新密码</div>' .
                        '<div style="font-size:1.8rem;font-weight:800;color:#e65100;letter-spacing:4px;font-family:monospace">' . htmlspecialchars($newPass) . '</div>' .
                        '</div>' .
                        '<p style="color:#666;font-size:0.9rem">请尽快登录系统并在"安全设置"中修改为您自己的密码。</p></div>';
        ...
        // AJAX 背景图切换
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'select_bg') {
            $path = trim($_POST['bg_path'] ?? '');
            $pdo = \App\Service\Database::getConnection();
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM bg_images WHERE file_path = ?");
            $stmt->execute([$path]);
            if ($stmt->fetchColumn() > 0) {
                $pdo->prepare("UPDATE system_settings SET bg_image_path = ? WHERE id = 1")->execute([$path]);
                \App\Model\SystemSetting::$cache = null; // 清除缓存
                $this->json(['ok' => true]);
            } else {
                $this->json(['ok' => false, 'error' => '图片不存在']);
            }
            exit;
        }
