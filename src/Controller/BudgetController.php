...
class BudgetController
{
    ...
    public function index(): void
    {
        ...
        if (isset($_GET['action'])) {
            ...
            // 获取某一项目在去年同月的预算金额（用于“复制去年同月”按钮）
            if ($action === 'get_prev_budget') {
            ...
            // 获取「上个月该项目（分类 / 项目）支出和收入汇总」以及「去年同月预算金额（无则为 0）」
            if ($action === 'get_budget_stats') {
                $type = $_GET['type'] ?? 'expense';
                $categoryId = isset($_GET['category_id']) && $_GET['category_id'] !== '' ? (int)$_GET['category_id'] : null;
                $itemId = isset($_GET['item_id']) && $_GET['item_id'] !== '' ? (int)$_GET['item_id'] : null;

                // 去年同月该项目的预算金额，若不存在则为 0
                $prevBudgetRow = Budget::findOneByUserYearMonthKey($userId, $prevYear, $prevMonth, $type, $categoryId, $itemId);
                $prevYearBudgetAmount = $prevBudgetRow ? (float)$prevBudgetRow['amount'] : 0.0;

                // 计算「上个月」的年月（相对于当前选择的预算年月）
                if ($month === 1) {
                ...
                $dateFrom = sprintf('%04d-%02d-01', $lastMonthYear, $lastMonth);
                $dateTo = date('Y-m-d', strtotime($dateFrom . ' +1 month -1 day'));

                // 汇总上个月该项目的收支及转账（不限定 type，分别统计 income / expense / transfer）
                $summary = Transaction::summarize($userId, [
                ...
                $prevMonthIncome = isset($summary['income']) ? (float)$summary['income'] : 0.0;
                $prevMonthExpense = isset($summary['expense']) ? (float)$summary['expense'] : 0.0;
                $prevMonthTransfer = isset($summary['transfer']) ? (float)$summary['transfer'] : 0.0;
                ...
                echo json_encode([
                    ...
                    'prev_month' => [
                        ...
                        'transfer' => $prevMonthTransfer,
                    ],
                    'prev_year_budget' => $prevYearBudgetAmount,
                ], JSON_UNESCAPED_UNICODE);
                return;
            }
        }
