...
class ReportController
{
    ...
    public function index(): void
    {
        ...
        if ($compareLastYear) {
            ...
            $stmtLast = $pdo->prepare('SELECT DATE(trans_time) AS d, type, COALESCE(SUM(amount),0) AS total
                FROM transactions
                WHERE user_id = :uid AND trans_time BETWEEN :from AND :to
                GROUP BY DATE(trans_time), type
                ORDER BY d');
            ...
            // 如果当前没有数据但去年有数据，用去年的日期作为标签
            if (empty($byDate) && !empty($byDateLast)) {
                ksort($byDateLast);
                foreach ($byDateLast as $dLast => $v) {
                    // 将去年日期转换为今年日期作为标签
                    $dt = new \DateTime($dLast);
                    $dThisYear = $dt->modify('+1 year')->format('Y-m-d');
                    $labels[] = $dThisYear;
                    $incomeData[] = 0.0;
                    $expenseData[] = 0.0;
                    $incomeLastData[] = (float)$v['income'];
                    $expenseLastData[] = (float)$v['expense'];
                }
            } else {
                foreach ($byDate as $d => $v) {
                    $dt = new \DateTime($d);
                    $dLast = $dt->modify('-1 year')->format('Y-m-d');
                    $incomeLastData[] = isset($byDateLast[$dLast]) ? (float)$byDateLast[$dLast]['income'] : 0.0;
                    $expenseLastData[] = isset($byDateLast[$dLast]) ? (float)$byDateLast[$dLast]['expense'] : 0.0;
                }
            }
        }
