...
class Account
{
    public static function allByLedger(int $ledgerId): array
    {
        ...
        $stmt = $pdo->prepare('SELECT a.*, ag.name AS group_name, ag.code AS group_code FROM accounts a JOIN account_groups ag ON a.group_id = ag.id WHERE a.ledger_id = :lid ORDER BY ag.id, a.id');
    ...
    public static function findByLedger(int $ledgerId, int $accountId): ?array
    {
        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('SELECT * FROM accounts WHERE id = :id AND ledger_id = :lid LIMIT 1');
        $stmt->execute([':id' => $accountId, ':lid' => $ledgerId]);
    ...
    public static function createForLedger(int $userId, int $ledgerId, int $groupId, string $name, ?string $accountNo, float $initialBalance, ?string $iconType = null, ?string $iconValue = null): void
    {
        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('INSERT INTO accounts (user_id, ledger_id, group_id, name, account_no, initial_balance, current_balance, icon_type, icon_value) VALUES (:uid,:lid,:gid,:name,:no,:init,:curr,:icon_type,:icon_value)');
        $stmt->execute([
            ':uid' => $userId,
            ':lid' => $ledgerId,
            ':gid' => $groupId,
    ...
    public static function updateForLedger(int $ledgerId, int $accountId, int $groupId, string $name, ?string $accountNo, ?string $iconType = null, ?string $iconValue = null): void
    {
        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('UPDATE accounts SET group_id = :gid, name = :name, account_no = :no, icon_type = :icon_type, icon_value = :icon_value WHERE id = :id AND ledger_id = :lid');
        $stmt->execute([
            ':gid' => $groupId,
            ':name' => $name,
            ':no' => $accountNo,
    ...
    public static function deleteForLedger(int $ledgerId, int $accountId): bool
    {
        $pdo = Database::getConnection();
        // 检查是否有流水（同账本内）
        $stmt = $pdo->prepare('SELECT COUNT(*) FROM transactions WHERE ledger_id = :lid AND (from_account_id = :id OR to_account_id = :id)');
        $stmt->execute([':lid' => $ledgerId, ':id' => $accountId]);
        if ((int)$stmt->fetchColumn() > 0) {
            return false;
        }
        $stmt = $pdo->prepare('DELETE FROM accounts WHERE id = :id AND ledger_id = :lid');
        $stmt->execute([':id' => $accountId, ':lid' => $ledgerId]);
