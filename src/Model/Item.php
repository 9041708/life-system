...
class Item
{
    public static function findByLedger(int $ledgerId, int $id): ?array
    {
        ...
        $stmt = $pdo->prepare('SELECT * FROM items WHERE id = :id AND ledger_id = :lid LIMIT 1');
    ...
    public static function allByLedger(int $ledgerId, ?int $categoryId = null): array
    {
        $pdo = Database::getConnection();
        if ($categoryId) {
            $stmt = $pdo->prepare('SELECT * FROM items WHERE ledger_id = :lid AND category_id = :cid ORDER BY sort_order, id');
            $stmt->execute([':lid' => $ledgerId, ':cid' => $categoryId]);
        } else {
            $stmt = $pdo->prepare('SELECT * FROM items WHERE ledger_id = :lid ORDER BY category_id, sort_order, id');
    ...
    public static function createForLedger(int $userId, int $ledgerId, int $categoryId, string $name, int $sortOrder = 0, ?string $iconType = null, ?string $iconValue = null): void
    {
        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('INSERT INTO items (user_id, ledger_id, category_id, name, sort_order, icon_type, icon_value) VALUES (:uid,:lid,:cid,:name,:sort,:icon_type,:icon_value)');
        try {
            $stmt->execute([
                ':uid' => $userId,
                ':lid' => $ledgerId,
    ...
    public static function updateForLedger(int $ledgerId, int $id, int $categoryId, string $name, int $sortOrder = 0, ?string $iconType = null, ?string $iconValue = null): void
    {
        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('UPDATE items SET category_id = :cid, name = :name, sort_order = :sort, icon_type = :icon_type, icon_value = :icon_value WHERE id = :id AND ledger_id = :lid');
        try {
            $stmt->execute([
                ':cid' => $categoryId,
                ':name' => $name,
    ...
    {
        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('SELECT COUNT(*) FROM transactions WHERE ledger_id = :lid AND item_id = :id');
        $stmt->execute([':lid' => $ledgerId, ':id' => $id]);
        if ((int)$stmt->fetchColumn() > 0) {
            return false;
        }
        $stmt = $pdo->prepare('DELETE FROM items WHERE id = :id AND ledger_id = :lid');
        $stmt->execute([':id' => $id, ':lid' => $ledgerId]);
