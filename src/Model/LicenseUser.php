...
{
    /**
     * 统一规范域名字符串，忽略协议 / 路径 / www 前缀，仅用于比较。
     */
    ...
    private static function ensureTableExists(): void
    {
        ...
        try {
            $pdo->query('SELECT 1 FROM license_users LIMIT 1');
        } catch (\Throwable $e) {
            $sql = 'CREATE TABLE IF NOT EXISTS license_users (
                ...
                license_period VARCHAR(50) NULL,
                domain_change_quota INT NULL,
                domain_change_used INT NULL,
    ...
    public static function findByCode(string $licenseCode): ?array
    {
        self::ensureTableExists();
        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('SELECT * FROM license_users WHERE license_code = :code ORDER BY id DESC LIMIT 1');
        $stmt->execute([
            ':code' => $licenseCode,
        ]);
        $row = $stmt->fetch(\PDO::FETCH_ASSOC);
    ...
    public static function findByEmailAndDomain(string $email, string $domain): ?array
    {
        self::ensureTableExists();
        $pdo = Database::getConnection();
        // 为了兼容历史数据中可能带有 http(s) 前缀、路径或 www. 前缀的写法，
        // 这里只按邮箱取出若干条，再在 PHP 中做规范化后的精确匹配。
        $stmt = $pdo->prepare('SELECT * FROM license_users WHERE email = :email ORDER BY id DESC LIMIT 50');
        $stmt->execute([
            ':email' => $email,
        ]);
        ...
        foreach ($rows as $row) {
            ...
            if ($target !== '' && self::normalizeDomain($rowDomain) === $target) {
                return $row;
            }
        }
