...
class Budget
{
    public static function listByLedgerMonth(int $ledgerId, int $year, int $month): array
    {
        ...
        $stmt = $pdo->prepare('SELECT b.*, 
            ...
            WHERE b.ledger_id = :lid AND b.year = :y AND b.month = :m
            ORDER BY b.type, b.category_id, b.item_id');
        $stmt->execute([':lid' => $ledgerId, ':y' => $year, ':m' => $month]);
    ...
    public static function upsertForLedger(int $userId, int $ledgerId, int $year, int $month, string $type, ?int $categoryId, ?int $itemId, float $amount): void
    {
        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('INSERT INTO budgets (user_id, ledger_id, year, month, type, category_id, item_id, amount)
            VALUES (:uid,:lid,:y,:m,:type,:cid,:iid,:amount)
