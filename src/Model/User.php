...
class User
{
    private static function ensureSystemIconSyncColumn(): void
    {
        $pdo = Database::getConnection();
        try {
            $colStmt = $pdo->query("SHOW COLUMNS FROM users LIKE 'system_icon_last_sync_change_id'");
        ...
        try {
            $pdo->exec("ALTER TABLE users ADD COLUMN system_icon_last_sync_change_id INT NULL DEFAULT NULL");
        } catch (\Throwable $e) {
            // ignore if no permission
        }
    }

    public static function getSystemIconLastSyncChangeId(int $userId): int
    {
        self::ensureSystemIconSyncColumn();
        $pdo = Database::getConnection();
        try {
            $stmt = $pdo->prepare('SELECT system_icon_last_sync_change_id FROM users WHERE id = :id LIMIT 1');
    ...
    public static function setSystemIconLastSyncChangeId(int $userId, int $changeId): void
    {
        self::ensureSystemIconSyncColumn();
        $changeId = max(0, (int)$changeId);
        $pdo = Database::getConnection();
        try {
            $stmt = $pdo->prepare('UPDATE users SET system_icon_last_sync_change_id = :cid WHERE id = :id');
            $stmt->execute([':cid' => $changeId, ':id' => $userId]);
        } catch (\Throwable $e) {
            // ignore
        }
    }
    ...
    public static function listAllForAdminSelect(): array
    {
        $pdo = Database::getConnection();
    ...
    public static function listPage(int $page, int $pageSize): array
    {
        ...
        $limitSql = ' LIMIT ' . (int)$offset . ', ' . (int)$pageSize;
    ...
    /**
     * 更新用户是否启用账户间转账功能的开关。
     * 需要数据库中存在 enable_transfer TINYINT(1) 字段。
     */
    public static function updateTransferFeature(int $userId, bool $enabled): void
    {
        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('UPDATE users SET enable_transfer = :t WHERE id = :id');
        $stmt->execute([
            ':t' => $enabled ? 1 : 0,
            ':id' => $userId,
        ]);
    ...
    /**
     * 更新用户是否允许账户余额为负数的开关。
     * 需要数据库中存在 allow_negative_balance TINYINT(1) 字段。
     */
    public static function updateAllowNegativeBalance(int $userId, bool $enabled): void
    {
        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('UPDATE users SET allow_negative_balance = :b WHERE id = :id');
        $stmt->execute([
            ':b' => $enabled ? 1 : 0,
            ':id' => $userId,
        ]);
